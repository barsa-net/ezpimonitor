...
ob_start();
require('rpi_rev/raspberry_pi_revision.php');
$info = ob_get_clean();
...
$datas = array(
    'model'         => json_decode($info, TRUE)["model"],
    'soc'           => json_decode($info, TRUE)["processor"],
    'cpu'           => $model,
    'num_cores'     => $num_cores,
    'frequency'     => $frequency,
    'manufacturer'  => json_decode($info, TRUE)["manufacturer"],
    'warrantyvoid'  => ucfirst(json_decode($info, TRUE)["warrantyVoid"]),
    'temp'          => $temp,
);
